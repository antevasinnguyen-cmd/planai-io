...
export default function SignupPage() {
  ...
  const handleGoogleSignUp = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.log('=== SIGNUP: Bắt đầu đăng ký với Google ===')
      ...
      if (redirectedFrom) {
        ...
        console.log('=== SIGNUP: Lưu đường dẫn chuyển hướng ===', redirectedFrom)
      }
      
      // Đăng ký bằng Google (thực tế là đăng nhập vì Supabase tự động tạo tài khoản nếu chưa có)
      console.log('=== SIGNUP: Gọi hàm signInWithGoogle ===')
      const result = await signInWithGoogle()
      
      if (result.error) {
        console.error('=== SIGNUP: Lỗi đăng ký Google ===', result.error)
        setError(`Đăng ký bằng Google thất bại: ${result.error.message || 'Vui lòng thử lại.'}`)
        setIsLoading(false)
      } else {
        console.log('=== SIGNUP: Đăng ký Google thành công, đợi chuyển hướng ===')
        // Để signInWithGoogle tự động chuyển hướng đến Google OAuth
        // Không cần setIsLoading(false) vì sẽ chuyển trang
      }
    } catch (error: any) {
      console.error('=== SIGNUP: Lỗi không xác định khi đăng ký Google ===', error)
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-blue-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-2 text-3xl font-bold text-gray-900">Tạo tài khoản mới</h2>
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm font-medium">Đăng ký với Google</span>
            </div>
          </button>
        </div>
