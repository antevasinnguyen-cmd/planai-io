...
function extractTuViInfo(text: string): { full_name: string | null, birth_time: string | null, gender: string | null } {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // CRITICAL: Extract Tu Vi info (full_name, birth_time, gender)
    const tuViInfo = extractTuViInfo(fullChatSummary)
    ...
    const extracted = {
      ...
      // CRITICAL: Tu Vi fields
      full_name: tuViInfo.full_name || collectedInfo?.full_name || collectedInfo?.userProfile?.full_name || undefined,
      birth_time: tuViInfo.birth_time || collectedInfo?.birth_time || collectedInfo?.userProfile?.birth_time || undefined,
      gender: tuViInfo.gender || collectedInfo?.gender || collectedInfo?.userProfile?.gender || undefined
    }
    
    logger.info('FAST_GENERATE_TUVI_INFO', {
