...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = [
      ...
      '  Tiêu đề; Tổng quan (mục tiêu + hồ sơ cá nhân & bối cảnh); SWOT cá nhân (Việt Nam); Mục tiêu SMART;',
      '  Phân tích mục tiêu tài chính & chiến lược tổng quan; Sơ đồ lộ trình/mindmap (đặt code vào mermaid_blocks);',
      '  Đề xuất chiến lược cụ thể + đi sâu chi tiết; Những việc cần làm để đạt mục tiêu; Yếu tố khách quan/chủ quan;',
      '  Kỹ năng/kinh nghiệm cần có; Kế hoạch tích luỹ tài sản; Kế hoạch đầu tư & quản trị rủi ro;',
