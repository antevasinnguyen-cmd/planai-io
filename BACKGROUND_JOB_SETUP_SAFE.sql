-- ============================================
-- BACKGROUND JOB SYSTEM - Supabase SQL Setup (SAFE VERSION)
-- ============================================
-- This version includes DROP IF EXISTS to avoid conflicts
...
-- Drop existing indexes if they exist (safe)
DROP INDEX IF EXISTS idx_plan_jobs_user_id;
DROP INDEX IF EXISTS idx_plan_jobs_status;
DROP INDEX IF EXISTS idx_plan_jobs_created_at;
...
-- 3. Drop existing trigger if exists (safe)
...
-- 4. Drop existing functions if they exist (safe)
DROP FUNCTION IF EXISTS update_subscription_timestamp();
DROP FUNCTION IF EXISTS update_plan_jobs_timestamp();

-- Update timestamp on subscription changes
...
-- Update timestamp on plan_jobs changes
...
-- 5. Enable RLS (Row Level Security) for security
ALTER TABLE plan_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE subscriptions ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist (safe)
DROP POLICY IF EXISTS "Users can view their own jobs" ON plan_jobs;
DROP POLICY IF EXISTS "Users can insert their own jobs" ON plan_jobs;
...
-- 7. Verify tables
SELECT 'plan_jobs' as table_name, COUNT(*) as record_count FROM plan_jobs
UNION ALL
